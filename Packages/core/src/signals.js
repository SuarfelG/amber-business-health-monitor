...
exports.evaluateRevenueSignal = evaluateRevenueSignal;
exports.evaluateLeadsSignal = evaluateLeadsSignal;
exports.evaluateShowRateSignal = evaluateShowRateSignal;
exports.evaluateRefundRateSignal = evaluateRefundRateSignal;
/**
 * Revenue Signal (40% weight)
 * Compares net revenue trend YoY or period-over-period
 */
function evaluateRevenueSignal(current, previous) {
    if (!current || !previous)
        return null;
    const currentRev = current.netRevenue;
    const previousRev = previous.netRevenue;
    // Both periods have no revenue
    if (currentRev === 0 && previousRev === 0)
        return null;
 ...
 */
function evaluateLeadsSignal(current, previous) {
    if (!current || !previous)
        return null;
 ...
 */
function evaluateShowRateSignal(current) {
    if (!current)
        return null;
    // Skip if not enough bookings to be meaningful
    if (current.appointmentsBooked < 3)
        return null;
 ...
 */
function evaluateRefundRateSignal(current) {
    if (!current)
        return null;
    // Skip if not enough charges to be meaningful
    if (current.chargeCount < 5)
        return null;
