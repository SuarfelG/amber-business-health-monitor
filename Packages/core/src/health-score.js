...
function computeHealthScore(input) {
    ...
    const signals = [];
    const revenueSignal = (0, signals_1.evaluateRevenueSignal)(input.current.revenue, input.previous.revenue);
    if (revenueSignal)
        signals.push(revenueSignal);
    const leadsSignal = (0, signals_1.evaluateLeadsSignal)(input.current.crm, input.previous.crm);
    if (leadsSignal)
        signals.push(leadsSignal);
    const showRateSignal = (0, signals_1.evaluateShowRateSignal)(input.current.crm);
    if (showRateSignal)
        signals.push(showRateSignal);
    const refundRateSignal = (0, signals_1.evaluateRefundRateSignal)(input.current.revenue);
    if (refundRateSignal)
        signals.push(refundRateSignal);
